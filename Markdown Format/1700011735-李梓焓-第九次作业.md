...
$$
其中$\mathrm{coef}[\chi_i(\boldsymbol{x}_1)]$为提出$\chi_i(\boldsymbol{x}_1)$后的代数余子式，我们有：
$$
\begin{split} 
...
&= (-1)^{\mathcal{P}_{ij}} N \idotsint \frac{1}{\sqrt{N!}} \Big\{ \phi_m(\boldsymbol{x}_1) \mathrm{coef}[\phi_m(\boldsymbol{x}_1)]+\sum\limits_{i=1}^{N-1} (-1)^{2+i} \phi_{l_i}(\boldsymbol{x}_1) \mathrm{coef}[\phi_{l_i}(\boldsymbol{x}_1)] \Big\} \\ &\quad \cdot \frac{1}{\sqrt{N!}} \Big\{ \phi_n^{*}(\boldsymbol{x}_1^{'}) \mathrm{coef}[\phi_n^{*}(\boldsymbol{x}_1^{'})]+\sum\limits_{j=2}^{N} (-1)^{2+j} \phi_{l_j}^{*}(\boldsymbol{x}_1^{'}) \mathrm{coef}[\phi_{l_j}^{*}(\boldsymbol{x}_1^{'})] \Big\} d \boldsymbol{x}_2 d \boldsymbol{x}_3 \dots d \boldsymbol{x}_N \\
&= (-1)^{\mathcal{P}_{ij}} \frac{1}{(N-1)!} \idotsint \phi_m(\boldsymbol{x}_1) \phi_n^{*}(\boldsymbol{x}_1^{'}) \mathrm{coef}[\phi_m(\boldsymbol{x}_1)] \mathrm{coef}[\phi_n^{*}(\boldsymbol{x}_1^{'})] d \boldsymbol{x}_2 d \boldsymbol{x}_3 \dots d \boldsymbol{x}_N （利用正交性） \\
