...
$$
现在考虑$\hat{S}_{-} \hat{S}_{+}$的作用效果，显然$\hat{S}_{+}=\sum\limits_{u} \hat{s}_{u,+}$，$\hat{S}_{-}=\sum\limits_{u} \hat{s}_{u,-}$，因此
